...
namespace Zeze.Services
{
    public class Constant
    {
        ...
        public const int eEncryptTypeRsaAes = 3;
    ...
    public class HandshakeBase : Service
    {
        ...
        private Task<long> ProcessCHandshake(Protocol _p)
        {
            try
            {
                ...
                switch (p.Argument.EncryptType)
                {
                    case Constant.eEncryptTypeAes:
                    {
                        ...
                        */
                        ...
                        inputKey = inputIv = Digest.Md5(inputKey);
                        outputKey = outputIv = Digest.Md5(outputKey);
                        break;
                    }
                ...
                ((TcpSocket)p.Sender).SetInputSecurityCodec(inputKey, inputIv, c2s);
                ...
                ((TcpSocket)p.Sender).SetOutputSecurityCodec(outputKey, outputIv, s2c);
                        ...
                        switch (p.Argument.EncryptType)
                        {
                            case Constant.eEncryptTypeAes:
                            {
                                ...
                                byte[] material = Handshake.Helper.ComputeDHKey(
                                    ...
                                    (BigInteger)ctx.context).ToByteArray();
                                Array.Reverse(material);
                            ...
                            case Constant.eEncryptTypeRsaAes:
                            {
                                ...
                                inputIv = new byte[16];
                                outputKey = new byte[16];
                                outputIv = new byte[16];
                                var secret = (byte[])ctx.context;
                                Buffer.BlockCopy(secret, 0, outputIv, 0, 16);
                                Buffer.BlockCopy(secret, 16, outputKey, 0, 16);
                                Buffer.BlockCopy(secret, 32, inputIv, 0, 16);
                                Buffer.BlockCopy(secret, 48, inputKey, 0, 16);
                                break;
                            }
                        }

                        ((TcpSocket)p.Sender).SetOutputSecurityCodec(outputKey, outputIv, p.Argument.CompressC2s);
                        ((TcpSocket)p.Sender).SetInputSecurityCodec(inputKey, inputIv, p.Argument.CompressS2c);
        ...
        protected void StartHandshake(Handshake.SHandshake0Argument arg, AsyncSocket so)
        {
            try
            {
                ...
                switch (arg.EncryptType)
                {
                    case Constant.eEncryptTypeAes:
                    case Constant.eEncryptTypeAesNoSecureIp:
                        var randomBigInt = Handshake.Helper.MakeDHRandom();
                        context = randomBigInt;
                        encryptParam = Handshake.Helper.GenerateDHResponse(1, randomBigInt).ToByteArray();
                        Array.Reverse(encryptParam);
                        break;
                    case Constant.eEncryptTypeRsaAes:
                        var rsaPubKey = Config.HandshakeOptions.RsaPubKey;
                        if (rsaPubKey == null)
                            throw new Exception("need RsaPubKey in ServiceConf");
                        var random64 = Handshake.Helper.MakeRandValues(64); // inputIv + inputKey + outputIv + outputKey
                        context = random64;
                        encryptParam = rsaPubKey.Encrypt(random64, RSAEncryptionPadding.Pkcs1);
                        break;
                    default:
                        context = BigInteger.Zero;
                        encryptParam = Array.Empty<byte>();
                        break;
                }
