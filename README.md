# 目标
	...
	0) 尽可能减少实现业务逻辑时需要的技术手段。
	   让实现代码离业务逻辑尽可能近，几乎能对照起来。
	1) 解决扩容问题，可以很容易构建成千上万台机器集群的服务。
	2) 解决高可用性问题，达到7x24小时不间断工作。
	3) 简单直接的编程接口，直接融入编程语言。
...
# 主要特性
	...
	1) 缓存同步
	   跟 CPU 的 MESI（缓存一致性协议）类似，提供缓存同步。
	   这个功能由底层实现，应用出了配置不需要访问任何接口。
