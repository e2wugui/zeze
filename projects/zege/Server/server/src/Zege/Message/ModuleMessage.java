...
public class ModuleMessage extends AbstractModule {
    ...
    private long saveDepartmentMessage(Zege.Message.SendDepartmentMessage r) {
    ...
    protected long ProcessSendDepartmentMessageRequest(Zege.Message.SendDepartmentMessage r) throws Exception {
        ...
        ProviderUserSession.get(r); // verify session only
    ...
    private boolean calculateMessageRange(OutLong from, OutLong to, BMessageRoot messageRoot) {
        if (from.value == eGetMessageFromAboutRead) {
            from.value = messageRoot.getFirstMessageId();
