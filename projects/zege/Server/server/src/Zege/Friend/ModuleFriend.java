...
public class ModuleFriend extends AbstractModule {
	private static final Logger logger = LogManager.getLogger(ModuleFriend.class);
	...
	private static void accountToDepartment(String account, OutObject<String> groupId, OutLong departmentId) {
		var idx = account.indexOf("@group");
		var name0 = account.substring(0, idx);
		var nd = name0.split("@");
		if (nd.length > 1) {
			// 编码了部门编号的群名。
			groupId.value = nd[0] + "@group";
			departmentId.value = Long.parseLong(nd[1]);
		} else {
			// 正常群名。
			groupId.value = account;
			departmentId.value = 0;
		}
	}

	private static String departmentToAccount(String group, long departmentId) {
		if (departmentId == 0)
			return group;
		var idx = group.indexOf('@');
		var name = group.substring(0, idx);
		return name + "@" + departmentId + "@group";
	}
	...
	protected long ProcessAddDepartmentMemberRequest(Zege.Friend.AddDepartmentMember r) {
		...
		if (r.Argument.getDepartmentId() == 0) {
			...
			//} else {
				...
				getFriends(r.Argument.getAccount()).getOrAdd(r.Argument.getGroup());
			//}
		} else {
			if (null == groupMember)
	...
	private void quitMember(DepartmentTree<BManager, BGroupMember, BDepartmentMember, BGroupData, BDepartmentData> group,
								   String account, String groupId, long departmentId) {
		var groupMembers = group.getGroupMembers();
		var groupMember = groupMembers.get(account);
		if (groupMember != null) {
			var self = getFriends(account);
			var top = getTopmosts(account);
			if (departmentId == 0) {
				for (var belong : groupMember.getBelongDepartments()) {
					group.getDepartmentMembers(belong).remove(account);
					var departmentAccount = departmentToAccount(groupId, departmentId);
					self.remove(departmentAccount);
					top.remove(departmentAccount);
				}
	...
	@Override
	protected long ProcessCreateGroupRequest(Zege.Friend.CreateGroup r) throws Exception {
		if (r.Argument.getMembers().size() > App.ZegeConfig.GroupInviteMax)
			return errorCode(eGroupInviteMax);
		...
		if (null != user) {
			...
			if (!Cert.verifySignRsa(publicKey, r.Argument.getRandomData().bytesUnsafe(), r.Argument.getSigned().bytesUnsafe()))
				return errorCode(eVerifySigned);

			user.setCreateTime(System.currentTimeMillis());
			user.setState(BUser.StateCreated);

			var certEncoded = App.Zege_User.generateCert(groupId, publicKey);
			...
			r.Result.setGroup(groupId);

			var group = getGroup(groupId);
			group.create().setRoot(session.getAccount());
			var members = group.getGroupMembers();

//			if (r.Argument.getMembers().size() > App.ZegeConfig.GroupInviteLimit) {
//				// todo 超出邀请限制，需要走审批流程。
//			}

			for (var member : r.Argument.getMembers()) {
				if (null == App.Zege_User.get(member))
					continue; // skip not exist member
				members.put(member, new BGroupMember());
				getFriends(member).getOrAdd(groupId);
			}
			if (!r.Argument.getMembers().contains(session.getAccount())) {
	...
	protected long ProcessDeleteFriendRequest(Zege.Friend.DeleteFriend r) {
		...
		if (r.Argument.getAccount().endsWith("@group")) {
			var groupId = new OutObject<String>();
			var departmentId = new OutLong();
			accountToDepartment(r.Argument.getAccount(), groupId, departmentId);
			var group = getGroup(groupId.value);
			quitMember(group, session.getAccount(), groupId.value, departmentId.value);
		} else {
    ...
    @Override
    protected long ProcessGetPublicUserInfosRequest(Zege.Friend.GetPublicUserInfos r) {
		var session = ProviderUserSession.get(r);
		for (var account : r.Argument.getAccounts()) {
			var user = App.Zege_User.get(account);
			if (null == user)
				continue;

			var info = new BPublicUserInfo();
			info.setAccount(account);
			info.setNick(user.getNick());
			info.setLastCertIndex(user.getLastCertIndex());
			info.setCert(user.getCert());
