...
public class TestMoveMmo {
	...
	public void testMoveBench() throws Exception {
		...
		var map = new CubeMap(App.Instance.Demo_MyWorld.getWorld().getMapManager(), 0, 128, 128);
		...
		server.AddFactoryHandle(new metagame.builtin.World.Command().getTypeId(), new Service.ProtocolFactoryHandle<>(
				metagame.builtin.World.Command::new, this::ProcessMove));
		...
		for (int i = 0; i < moveBenchCount; ++i) {
			try (var ignoredGuard = new LockGuard(cubes2d)) {
				var move = new metagame.builtin.World.Command();
	...
	private long ProcessMove(metagame.builtin.World.Command move) {
