...
namespace Zeze.Transaction
{
    public class Procedure
    {
        ...
        public async Task<long> CallAsync()
        {
            ...
            {
                currentT.Rollback();
                LogAction?.Invoke(e, ResultCode.Exception, this, "");
#if ENABLE_STATISTICS
                ProcedureStatistics.Instance.GetOrAdd(ActionName).GetOrAdd(ResultCode.Exception).IncrementAndGet();
#endif
...
#endif
                return e is TaskCanceledException ? ResultCode.CancelException : ResultCode.Exception; // 回滚当前存储过程，不中断事务，外层存储过程判断结果自己决定是否继续。
            }
            finally
            {
