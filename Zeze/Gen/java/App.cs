...
namespace Zeze.Gen.java
{
    public class App
    {
        ...
        public void Make()
        {
            ...
            sw.WriteLine("    // private static final Logger logger = LogManager.getLogger(App.class);");
            sw.WriteLine("    public static final App instance = new App();");
            sw.WriteLine();
            sw.WriteLine("    public void start() throws Exception {");
            sw.WriteLine("        createZeze(Config.load(\"zeze.xml\"));");
            sw.WriteLine("        createService();");
            sw.WriteLine("        // new ProviderApp/LinkdApp");
            sw.WriteLine("        createModules();");
            sw.WriteLine("        Zeze.start(); // 启动数据库");
            sw.WriteLine("        startModules(); // 启动模块,装载配置");
            sw.WriteLine("        startService(); // 启动网络");
            sw.WriteLine("        // logger.info(\"server started!\");");
            sw.WriteLine("    }");
            sw.WriteLine();
            sw.WriteLine("    public void stop() throws Exception {");
            sw.WriteLine("        stopService(); // 关闭网络");
            sw.WriteLine("        stopModules(); // 关闭模块,卸载配置");
