            ...
            if (module.ReferenceService != null)
            {
                ...
                foreach (Protocol p in module.Protocols.Values)
                {
                    if (p is Rpc rpc)
                    {
                        ...
                        {
                            sw.WriteLine();
                            sw.WriteLine($"        protected abstract async System.Threading.Tasks.Task<long>  Process{namePrefix}" + rpc.Name + "Request(Zeze.Net.Protocol p);");
                        }
                        continue;
                    }
                    if (0 != (p.HandleFlags & serviceHandleFlags & Program.HandleCSharpFlags))
                    {
                        sw.WriteLine();
                        sw.WriteLine($"        protected abstract async System.Threading.Tasks.Task<long>  Process{namePrefix}" + p.Name + "(Zeze.Net.Protocol p);");
                    }
                }
            }
        }
