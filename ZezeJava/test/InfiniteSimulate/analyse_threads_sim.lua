-- 需要当前正在运行 simulate.bat 测试, 否则分析上次分析时保存的栈信息(stack_sim.log)
...
if pid_sim then
	...
	f = io.open("stack_sim.log", "wb")
	f:write(s)
	f:close()
else
	print "没有找到 Simulate 进程"
	f = io.open("stack_sim.log", "rb")
	if not f then return end
	print "从 stack_sim.log 读取线程栈信息"
	s = f:read "*a"
	f:close()
	print "------------------------------------------------------------"
end
...
local knowns = {
	{ "在Load等待Record1锁",                           ".Load(TableX.java:32)" },
	{ "在Load等待Acquire(Share)回复",                  ".Acquire(GlobalCacheManagerWithRaftAgent.java:172)", ".Load(TableX.java:89)" },
	...
	{ "在TableCache.CleanNow等待Acquire(Invalid)回复", ".Acquire(GlobalCacheManagerWithRaftAgent.java:172)", ".TryRemoveRecordUnderLocks(TableCache.java:228)" },
...
for _, thread in pairs(threads) do
	...
	if not isKnown then
		...
		if found then
			...
			if not f then f = io.open("stack_sim_others.log", "wb") end
			f:write(thread.text)
			f:write "\n\n"
		else
...
print "------------------------------------------------------------"
if counts[0] then
	print(string.format("%4d 个线程 未知 (通过stack_sim_others.log补充线程信息)", counts[0]))
end
if counts[-1] then
