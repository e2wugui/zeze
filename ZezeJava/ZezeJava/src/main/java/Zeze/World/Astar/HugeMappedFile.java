...
public class HugeMappedFile {
	...
	public byte get(long index) {
		...
		return mapped.array()[blockIndex];
	}

	public void set(long index, byte b) {
		var mapped = mapped(index);
		var blockIndex = (int)index & Integer.MAX_VALUE;
		mapped.array()[blockIndex] = b;
	}
	...
	private MappedByteBuffer mapped(long offset) {
		...
		var blockIndex = offset / Integer.MAX_VALUE;
		return mapped.computeIfAbsent(blockIndex, __ -> {
			try {
				// 每一个mmap块的size为2G，除了最后一块。
				var position = blockIndex * Integer.MAX_VALUE; // 当前这一块映射的起始位置。
				var wishEndSize = position + Integer.MAX_VALUE; // 当前这一块的【期望】结束位置。
				var size = wishEndSize < this.fileSize ? Integer.MAX_VALUE : (this.fileSize - position);
