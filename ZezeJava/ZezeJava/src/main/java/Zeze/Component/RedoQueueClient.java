...
public class RedoQueueClient extends Zeze.Services.HandshakeClient {
	public RedoQueueClient(String name, Zeze.Config config) throws Throwable {
		super(name, config);
	}
	...
	private void tryStartSendNextTask(BQueueTask add) throws RocksDBException {
		if (null != Pending)
			return;

		var remoteZezeServer = GetSocket(); // TODO random select
		if (null == remoteZezeServer)
			return;
		...
		if (LastDoneTaskId < LastTaskId) {
			...
			rpc.Send(remoteZezeServer, this::ProcessRunTaskResult);
			Pending = rpc;
		}
	}
