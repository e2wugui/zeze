...
/**
 * BSimpleTimer 的参数构造器，同时持有 BSimpleTimer 的运行时推进逻辑（下次触发时间计算）。
 ...
 *
 * <pre>
 * var timerId = timer.schedule(BSimpleTimerBuilder.ofDelay(1000)
 *         .period(5000).times(-1), MyHandle.class, customData);
 * </pre>
 */
public final class BSimpleTimerBuilder {
	...
	private BSimpleTimerBuilder(long delay) {
		if (delay < 0)
	 ...
	 */
	public static @NotNull BSimpleTimerBuilder ofDelay(long delay) {
		return new BSimpleTimerBuilder(delay);
	}
	 ...
	 */
	public @NotNull BSimpleTimerBuilder period(long period) {
		this.period = period;
		return this;
	}
	 ...
	 */
	public @NotNull BSimpleTimerBuilder times(long times) {
		if (times == 0)
	 ...
	 */
	public @NotNull BSimpleTimerBuilder endTime(long endTime) {
	 ...
	 */
	public @NotNull BSimpleTimerBuilder missfirePolicy(int missfirePolicy) {
		this.missfirePolicy = missfirePolicy;
		return this;
	}

	public @NotNull BSimpleTimerBuilder oneByOneKey(@NotNull String oneByOneKey) {
