...
public class TimerGameOnline {
	...
	public long schedule(long roleId, String cron, String name, Bean customData) {
		// 去掉下面两行，不允许在非登录状态注册timer。现在允许。
		//if (!online.isLogin(account, clientId))
		//	throw new IllegalStateException("not login. account=" + account + " clientId=" + clientId);
	...
	boolean cancel(long timerId) {
		...
		// remove online local
		online.removeLocalBean(bTimer.getRoleId(), LocalDataCustomPrefix + timerId);
	...
	private void fireCronLocal(long timerId, String name) {
		...
		Task.Call(online.ProviderApp.Zeze.NewProcedure(() -> {
			...
			var customData = online.getLocalBean(bTimer.getRoleId(), LocalDataCustomPrefix + timerId);
