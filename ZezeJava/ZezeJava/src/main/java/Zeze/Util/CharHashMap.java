package Zeze.Util;

import java.util.Arrays;
import java.util.Objects;
import java.util.function.Consumer;
import java.util.function.IntConsumer;
import java.util.function.IntFunction;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;

public class CharHashMap<V> implements Cloneable {
	private int size;
	private char @NotNull [] keyTable;
	private @Nullable V @NotNull [] valueTable;
	private @Nullable V zeroValue;
	private boolean hasZeroValue;
	private final float loadFactor;
	private int threshold;
	private int mask;
	private int shift;

	public CharHashMap() {
		this(2, 0.8f);
	}

	public CharHashMap(int cap) {
		this(cap, 0.8f);
	}

	@SuppressWarnings("unchecked")
	public CharHashMap(int cap, float loadFactor) {
		if (loadFactor <= 0 || loadFactor >= 1)
			throw new IllegalArgumentException("invalid loadFactor: " + loadFactor);
		this.loadFactor = loadFactor;
		final int tableSize = tableSize(Math.max(cap, 0));
		threshold = (int)(tableSize * loadFactor);
		mask = tableSize - 1;
		shift = Long.numberOfLeadingZeros(mask);
		keyTable = new char[tableSize];
		valueTable = (V[])new Object[tableSize];
	}

	public CharHashMap(@NotNull CharHashMap<? extends V> map) {
		size = map.size;
		keyTable = map.keyTable.clone();
		valueTable = map.valueTable.clone();
		zeroValue = map.zeroValue;
		hasZeroValue = map.hasZeroValue;
		loadFactor = map.loadFactor;
		threshold = map.threshold;
		mask = map.mask;
		shift = map.shift;
	}

	private int tableSize(int cap) {
		cap = Math.min(Math.max((int)Math.ceil(cap / loadFactor), 2), 1 << 30);
		return 1 << (32 - Integer.numberOfLeadingZeros(cap - 1)); // [0,1<<30] => [0,1,2,4,8,...,1<<30]
	}

	private int hash(char key) {
		return (int)((key * 0x9E3779B97F4A7C15L) >>> shift);
	}

	public char @NotNull [] getKeyTable() {
		return keyTable;
	}

	public @Nullable Object @NotNull [] getValueTable() {
		return valueTable;
	}

	public boolean hasZeroValue() {
		return hasZeroValue;
	}

	public @Nullable V getZeroValue() {
		return zeroValue;
	}

	public float getLoadFactor() {
		return loadFactor;
	}

	public int capacity() {
		return mask + 1;
	}

	public int size() {
		return size;
	}

	public boolean isEmpty() {
		return size == 0;
	}

	public boolean containsKey(char key) {
		if (key == 0)
			return hasZeroValue;
		final char[] kt = keyTable;
		final int m = mask;
		for (int i = hash(key); ; i = (i + 1) & m) {
			final char k = kt[i];
			if (k == key)
				return true;
			if (k == 0)
				return false;
		}
	}

	public boolean containsValue(@Nullable V value) {
		if (value == null) {
			if (hasZeroValue && zeroValue == null)
				return true;
			final char[] kt = keyTable;
			final V[] vt = valueTable;
			for (int i = 0, n = kt.length; i < n; i++)
				if (kt[i] != 0 && vt[i] == null)
					return true;
		} else {
			if (hasZeroValue && value.equals(zeroValue))
				return true;
			final char[] kt = keyTable;
			final V[] vt = valueTable;
			for (int i = 0, n = kt.length; i < n; i++)
				if (kt[i] != 0 && value.equals(vt[i]))
					return true;
		}
		return false;
	}

	public @Nullable V get(char key) {
		if (key == 0)
			return hasZeroValue ? zeroValue : null;
		final char[] kt = keyTable;
		final int m = mask;
		for (int i = hash(key); ; i = (i + 1) & m) {
			final char k = kt[i];
			if (k == key)
				return valueTable[i];
			if (k == 0)
				return null;
		}
	}

	public V getOrDefault(char key, @Nullable V defaultValue) {
		if (key == 0)
			return hasZeroValue ? zeroValue : defaultValue;
		final char[] kt = keyTable;
		final int m = mask;
		for (int i = hash(key); ; i = (i + 1) & m) {
			final char k = kt[i];
			if (k == key)
				return valueTable[i];
			if (k == 0)
				return defaultValue;
		}
	}

	public @Nullable V put(char key, @Nullable V value) {
		if (key == 0) {
			final V oldV = zeroValue;
			zeroValue = value;
			if (!hasZeroValue) {
				hasZeroValue = true;
				size++;
			}
			return oldV;
		}
		final char[] kt = keyTable;
		final V[] vt = valueTable;
		final int m = mask;
		for (int i = hash(key); ; i = (i + 1) & m) {
			final char k = kt[i];
			if (k == 0) {
				kt[i] = key;
				vt[i] = value;
				if (++size >= threshold)
					resize(kt.length << 1);
				return null;
			}
			if (k == key) {
				final V oldV = vt[i];
				vt[i] = value;
				return oldV;
			}
		}
	}

	public void putAll(@NotNull CharHashMap<? extends V> map) {
		if (map.hasZeroValue) {
			hasZeroValue = true;
			zeroValue = map.zeroValue;
		}
		final char[] mapKt = map.keyTable;
		final V[] mapVt = map.valueTable;
		for (int i = 0, n = mapKt.length; i < n; i++) {
			final char k = mapKt[i];
			if (k != 0)
				put(k, mapVt[i]);
		}
	}

	public @Nullable V putIfAbsent(char key, @Nullable V value) {
		if (key == 0) {
			final V oldV = zeroValue;
			if (!hasZeroValue) {
				hasZeroValue = true;
				zeroValue = value;
				size++;
			}
			return oldV;
		}
		final char[] kt = keyTable;
		final V[] vt = valueTable;
		final int m = mask;
		for (int i = hash(key); ; i = (i + 1) & m) {
			final char k = kt[i];
			if (k == 0) {
				kt[i] = key;
				vt[i] = value;
				if (++size >= threshold)
					resize(kt.length << 1);
				return null;
			}
			if (k == key)
				return vt[i];
		}
	}

	public V computeIfAbsent(char key, @NotNull IntFunction<? extends V> mappingFunction) {
		if (key == 0) {
			V v = zeroValue;
			if (!hasZeroValue) {
				V newV = mappingFunction.apply(0);
				if (newV == null)
					return null;
				zeroValue = v = newV;
				hasZeroValue = true;
				size++;
			}
			return v;
		}
		final char[] kt = keyTable;
		final V[] vt = valueTable;
		final int m = mask;
		for (int i = hash(key); ; i = (i + 1) & m) {
			final char k = kt[i];
			if (k == 0) {
				V v = mappingFunction.apply(key);
				if (v == null)
					return null;
				kt[i] = key;
				vt[i] = v;
				if (++size >= threshold)
					resize(kt.length << 1);
				return v;
			}
			if (k == key)
				return vt[i];
		}
	}

	public @Nullable V replace(char key, @Nullable V value) {
		if (key == 0) {
			final V oldV = zeroValue;
			if (hasZeroValue)
				zeroValue = value;
			return oldV;
		}
		final char[] kt = keyTable;
		final V[] vt = valueTable;
		final int m = mask;
		for (int i = hash(key); ; i = (i + 1) & m) {
			final char k = kt[i];
			if (k == 0)
				return null;
			if (k == key) {
				final V oldV = vt[i];
				vt[i] = value;
				return oldV;
			}
		}
	}

	public boolean replace(char key, @Nullable V oldValue, @Nullable V newValue) {
		if (key == 0) {
			if (!hasZeroValue || !Objects.equals(oldValue, zeroValue))
				return false;
			zeroValue = newValue;
			return true;
		}
		final char[] kt = keyTable;
		final V[] vt = valueTable;
		for (int i = hash(key), m = mask; ; i = (i + 1) & m) {
			final char k = kt[i];
			if (k == key) {
				if (!Objects.equals(oldValue, vt[i]))
					return false;
				vt[i] = newValue;
				return true;
			}
			if (k == 0)
				return false;
		}
	}

	public interface CharObjectFunction<V> {
		@Nullable V apply(char key, V value);
	}

	public V compute(char key, @NotNull CharObjectFunction<V> op) {
		if (key == 0) {
			final V oldV = zeroValue;
			final V v = op.apply(key, oldV);
			if (v != oldV) {
				zeroValue = v;
				if (v == null) {
					if (hasZeroValue) {
						hasZeroValue = false;
						size--;
					}
				} else if (!hasZeroValue) {
					hasZeroValue = true;
					size++;
				}
			}
			return v;
		}
		final char[] kt = keyTable;
		final V[] vt = valueTable;
		for (int i = hash(key), m = mask; ; i = (i + 1) & m) {
			final char k = kt[i];
			if (k == key) {
				final V oldV = vt[i];
				final V v = op.apply(key, oldV);
				if (v != oldV) {
					vt[i] = v;
					if (v == null) {
						for (int j = (i + 1) & m; (key = kt[j]) != 0; j = (j + 1) & m) {
							final int h = hash(key);
							if (((j - h) & m) > ((i - h) & m)) {
								kt[i] = key;
								vt[i] = vt[j];
								i = j;
							}
						}
						kt[i] = 0;
						vt[i] = null;
						size--;
					}
				}
				return v;
			}
			if (k == 0) {
				final V v = op.apply(key, null);
				if (v != null) {
					kt[i] = key;
					vt[i] = v;
					if (++size >= threshold)
						resize(kt.length << 1);
				}
				return v;
			}
		}
	}

	public @Nullable V remove(char key) {
		if (key == 0) {
			if (!hasZeroValue)
				return null;
			hasZeroValue = false;
			final V oldV = zeroValue;
			zeroValue = null;
			size--;
			return oldV;
		}
		final char[] kt = keyTable;
		final V[] vt = valueTable;
		final int m = mask;
		int i;
		for (i = hash(key); ; i = (i + 1) & m) {
			final char k = kt[i];
			if (k == key)
				break;
			if (k == 0)
				return null;
		}
		final V oldV = vt[i];
		for (int j = (i + 1) & m; (key = kt[j]) != 0; j = (j + 1) & m) {
			final int h = hash(key);
			if (((j - h) & m) > ((i - h) & m)) {
				kt[i] = key;
				vt[i] = vt[j];
				i = j;
			}
		}
		kt[i] = 0;
		vt[i] = null;
		size--;
		return oldV;
	}

	public boolean remove(char key, @Nullable V value) {
		if (key == 0) {
			if (!hasZeroValue || !Objects.equals(value, zeroValue))
				return false;
			hasZeroValue = false;
			zeroValue = null;
			size--;
			return true;
		}
		final char[] kt = keyTable;
		final V[] vt = valueTable;
		final int m = mask;
		int i;
		for (i = hash(key); ; i = (i + 1) & m) {
			final char k = kt[i];
			if (k == key) {
				if (!Objects.equals(value, vt[i]))
					return false;
				break;
			}
			if (k == 0)
				return false;
		}
		for (int j = (i + 1) & m; (key = kt[j]) != 0; j = (j + 1) & m) {
			final int h = hash(key);
			if (((j - h) & m) > ((i - h) & m)) {
				kt[i] = key;
				vt[i] = vt[j];
				i = j;
			}
		}
		kt[i] = 0;
		vt[i] = null;
		size--;
		return true;
	}

	public void clear() {
		if (size == 0)
			return;
		size = 0;
		hasZeroValue = false;
		zeroValue = null;
		Arrays.fill(keyTable, (char)0);
		Arrays.fill(valueTable, null);
	}

	public void clear(int maxCap) {
		final int tableSize = tableSize(Math.max(maxCap, 0));
		if (tableSize >= keyTable.length) {
			clear();
			return;
		}
		size = 0;
		hasZeroValue = false;
		zeroValue = null;
		resize(tableSize);
	}

	public void shrink(int maxCap) {
		final int tableSize = tableSize(Math.max(maxCap, size));
		if (tableSize < keyTable.length)
			resize(tableSize);
	}

	public void ensureCapacity(int cap) {
		final int tableSize = tableSize(Math.max(cap, 0));
		if (tableSize > keyTable.length)
			resize(tableSize);
	}

	private void resize(int newSize) { // [1,2,4,8,...,0x4000_0000]
		threshold = (int)(newSize * loadFactor);
		final int m = newSize - 1;
		mask = m;
		shift = Long.numberOfLeadingZeros(m);
		final char[] kt = new char[newSize];
		@SuppressWarnings("unchecked") final V[] vt = (V[])new Object[newSize];
		if (size != 0) {
			final char[] oldKt = keyTable;
			final V[] oldVt = valueTable;
			for (int j = 0, n = oldKt.length; j < n; j++) {
				final char k = oldKt[j];
				if (k != 0) {
					for (int i = hash(k); ; i = (i + 1) & m) {
						if (kt[i] == 0) {
							kt[i] = k;
							vt[i] = oldVt[j];
							break;
						}
					}
				}
			}
		}
		keyTable = kt;
		valueTable = vt;
	}

	public void foreachKey(@NotNull IntConsumer consumer) {
		if (hasZeroValue)
			consumer.accept(0);
		for (final char k : keyTable)
			if (k != 0)
				consumer.accept(k);
	}

	public void foreachValue(@NotNull Consumer<V> consumer) {
		if (hasZeroValue)
			consumer.accept(zeroValue);
		final char[] kt = keyTable;
		final V[] vt = valueTable;
		for (int i = 0, n = kt.length; i < n; i++)
			if (kt[i] != 0)
				consumer.accept(vt[i]);
	}

	public interface CharObjectConsumer<V> {
		void accept(char key, V value);
	}

	public void foreach(@NotNull CharObjectConsumer<V> consumer) {
		if (hasZeroValue)
			consumer.accept((char)0, zeroValue);
		final char[] kt = keyTable;
		final V[] vt = valueTable;
		for (int i = 0, n = kt.length; i < n; i++) {
			final char k = kt[i];
			if (k != 0)
				consumer.accept(k, vt[i]);
		}
	}

	public interface CharObjectMapPredicate<V> {
		boolean test(@NotNull CharHashMap<V> map, char key, V value);
	}

	public boolean foreachTest(@NotNull CharObjectMapPredicate<V> tester) {
		if (hasZeroValue && !tester.test(this, (char)0, zeroValue))
			return false;
		final char[] kt = keyTable;
		final V[] vt = valueTable;
		for (int i = 0, n = kt.length; i < n; i++) {
			final char k = kt[i];
			if (k != 0 && !tester.test(this, k, vt[i]))
				return false;
		}
		return true;
	}

	public void foreachUpdate(@NotNull CharObjectFunction<V> func) {
		if (hasZeroValue) {
			final V oldV = zeroValue;
			final V v = func.apply((char)0, oldV);
			if (v != oldV) {
				zeroValue = v;
				if (v == null) {
					hasZeroValue = false;
					size--;
				}
			}
		}
		final char[] kt = keyTable;
		final V[] vt = valueTable;
		// 两遍：第一遍只应用func（更新值、记录返回null的待删除key），第二遍用remove统一做结构删除。
		// 原实现就地backward-shift删除会把同探测链的后续条目搬进已遍历区域，导致func跳过它们。
		char[] removedKeys = null;
		int removedCount = 0;
		for (int i = 0, n = kt.length; i < n; i++) {
			final char k = kt[i];
			if (k != 0) {
				final V oldV = vt[i];
				final V v = func.apply(k, oldV);
				if (v != oldV) {
					if (v == null) {
						if (removedKeys == null)
							removedKeys = new char[size()];
						removedKeys[removedCount++] = k;
					} else {
						vt[i] = v;
					}
				}
			}
		}
		for (int i = 0; i < removedCount; i++) {
			final char k = removedKeys[i];
			remove(k, get(k));
		}
	}

	public final class Iterator {
		private int idx = -2;

		public boolean moveToNext() {
			if (idx == -2) {
				idx = -1;
				if (hasZeroValue)
					return true;
			}
			final char[] kt = keyTable;
			for (final int lastIdx = kt.length - 1; idx < lastIdx; ) {
				if (kt[++idx] != 0)
					return true;
			}
			return false;
		}

		public char key() {
			return idx >= 0 ? keyTable[idx] : 0;
		}

		public V value() {
			return idx >= 0 ? valueTable[idx] : zeroValue;
		}
	}

	public @NotNull Iterator iterator() {
		return new Iterator();
	}

	@Override
	public @NotNull CharHashMap<V> clone() throws CloneNotSupportedException {
		@SuppressWarnings("unchecked") final CharHashMap<V> map = (CharHashMap<V>)super.clone();
		map.keyTable = keyTable.clone();
		map.valueTable = valueTable.clone();
		return map;
	}

	@Override
	public @NotNull String toString() {
		if (size == 0)
			return "{}";
		final StringBuilder sb = new StringBuilder(32).append('{');
		final char[] kt = keyTable;
		final V[] vt = valueTable;
		final int n = Math.min(kt.length, 20);
		int i = 0;
		if (hasZeroValue)
			sb.append("0=").append(zeroValue);
		else {
			for (; i < n; i++) {
				final char k = kt[i];
				if (k != 0) {
					sb.append(k).append('=').append(vt[i++]);
					break;
				}
			}
		}
		for (; i < n; i++) {
			final char k = kt[i];
			if (k != 0)
				sb.append(',').append(k).append('=').append(vt[i]);
		}
		if (n != kt.length)
			sb.append(",...");
		return sb.append('}').toString();
	}
}
