...
/**
 * 与 {@link CollMap1} 完全对应，但底层使用 {@link org.pcollections.PSortedMap}，
 * 因此 key 必须实现 {@link Comparable}。事务模型与 CollMap1 一致。
 */
public class CollSortedMap1<K extends Comparable<K>, V> extends CollSortedMap<K, V> {
	...
	public CollSortedMap1(Class<K> keyClass, Class<V> valueClass) {
		keyCodecFuncs = SerializeHelper.createCodec(keyClass);
		valueCodecFuncs = SerializeHelper.createCodec(valueClass);
		logTypeId = Zeze.Transaction.Bean.hash32("Zeze.Raft.RocksRaft.LogSortedMap1<"
				+ Reflect.getStableName(keyClass) + ", " + Reflect.getStableName(valueClass) + '>');
	}

	private CollSortedMap1(int logTypeId, SerializeHelper.CodecFuncs<K> keyCodecFuncs,
	...
	public void add(K key, V value) {
		if (isManaged()) {
			...
			var mapLog = (LogSortedMap1<K, V>)Transaction.getCurrent().logGetOrAdd(
	...
	public void leaderApplyNoRecursive(Log _log) {
		map = ((LogSortedMap1<K, V>)_log).getValue();
	}

	@Override
	public LogBean createLogBean() {
		var log = new LogSortedMap1<>(logTypeId, keyCodecFuncs, valueCodecFuncs);
