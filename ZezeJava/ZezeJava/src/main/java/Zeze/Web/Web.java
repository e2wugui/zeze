...
public class Web extends AbstractWeb {
	...
	private static final String COOKIE_SESSION_NAME = "ZEZEWEBSESSIONID=";
	...
	public String putSession(String account) {
		...
		return COOKIE_SESSION_NAME + sessionId;
	}
	...
	public BSession getSession(List<String> cookie) {
		if (cookie == null)
			return null;
		for (var c : cookie) {
			if (c.startsWith(COOKIE_SESSION_NAME))
				return _tSessions.get(c.substring(COOKIE_SESSION_NAME.length()));
		}
		return null;
	}
}
