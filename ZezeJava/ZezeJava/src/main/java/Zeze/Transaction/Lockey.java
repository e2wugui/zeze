...
public final class Lockey implements Comparable<Lockey> {
	...
	public void EnterReadLock() {
		...
		TableStatistics.getInstance().GetOrAdd(TableKey.getName()).getReadLockTimes().incrementAndGet();
	...
	public void EnterWriteLock() {
		if (!rwLock.isWriteLockedByCurrentThread()) // 第一次才计数
			TableStatistics.getInstance().GetOrAdd(TableKey.getName()).getWriteLockTimes().incrementAndGet();
	...
	public boolean TryEnterReadLock(int millisecondsTimeout) {
		...
		TableStatistics.getInstance().GetOrAdd(TableKey.getName()).getTryReadLockTimes().incrementAndGet();
	...
	public boolean TryEnterWriteLock(int millisecondsTimeout) {
		if (!rwLock.isWriteLockedByCurrentThread()) // 第一次才计数，即时失败了也计数，根据观察情况再决定采用那种方案。
			TableStatistics.getInstance().GetOrAdd(TableKey.getName()).getTryWriteLockTimes().incrementAndGet();
