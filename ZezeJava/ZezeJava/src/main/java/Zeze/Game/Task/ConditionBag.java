...
public class ConditionBag implements Condition {
	...
	public boolean accept(ConditionEvent event) {
		if (event instanceof Event) {
			...
			if (itemId == e.getItemId()) {
				count = event.getTaskModule().getBag().count(itemId);
	...
	@Override
	public boolean finish(TaskModule module) {
		var bag = module.getBag();
		if (bag.remove(itemId, expected))
			return true;
		// 当发生完成任务的时候扣除物品失败，意味着物品变成了不够的，更新一下本地的计数。
		// 最外面完成任务的时候会检查finish结果，失败的时候，会重新更新任务的状态。
		count = bag.count(itemId);
		return false;
	}
