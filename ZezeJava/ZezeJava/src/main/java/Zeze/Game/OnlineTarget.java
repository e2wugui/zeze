...
sealed interface OnlineTarget {
	...
	/** 把编码好的协议投递到本目标，立即执行。返回发送数（OnlineSpec.sendNow 透传给调用方；空目标 0，AllRoles 恒 -1）。 */
	...
	static int dispatch(@NotNull Online online, @NotNull Collection<Long> roleIds, long typeId,
		...
		if (size == 0)
			return 0;
		if (size == 1)
			return online.sendDirect(roleIds.iterator().next(), typeId, data, quietWhenAbsent) ? 1 : 0;
		return online.sendDirect(roleIds, typeId, data, quietWhenAbsent);
	}
	...
	record Role(long roleId) implements OnlineTarget {
		...
		public int send(@NotNull Online online, long typeId, @NotNull Binary data, boolean quietWhenAbsent) {
			return online.sendDirect(roleId, typeId, data, quietWhenAbsent) ? 1 : 0;
		}
	...
	record Roles(@NotNull Collection<Long> roleIds) implements OnlineTarget {
		...
		@Override
		public int send(@NotNull Online online, long typeId, @NotNull Binary data, boolean quietWhenAbsent) {
			return dispatch(online, roleIds, typeId, data, quietWhenAbsent);
		}
	...
	record AllRoles(@NotNull Collection<Long> roleIds) implements OnlineTarget {
		...
		public int send(@NotNull Online online, long typeId, @NotNull Binary data, boolean quietWhenAbsent) {
			...
			online.getProviderWithOnline().foreachOnline(o -> dispatch(o, roleIds, typeId, data, quietWhenAbsent));
			return -1; // 跨集合发送数无意义
		}
	...
	/** 可靠通知（Game 底层 sendReliableNotifyDirect 支持 quietWhenAbsent：缺席是否静默）。 */
	record Reliable(long roleId, @NotNull String listenerName) implements OnlineTarget {
		@Override
		public int send(@NotNull Online online, long typeId, @NotNull Binary data, boolean quietWhenAbsent) {
			online.sendReliableNotifyDirect(roleId, listenerName, typeId, data, quietWhenAbsent);
			return 1;
		}
