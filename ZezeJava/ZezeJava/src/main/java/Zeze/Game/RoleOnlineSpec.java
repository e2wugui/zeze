...
public final class RoleOnlineSpec extends OnlineSpec {
	...
	@Override
	public @NotNull RoleOnlineSpec quietWhenAbsent(boolean quietWhenAbsent) { // 协变返回，保持链式
	...
	void sendRpc(@NotNull Rpc<A, R> rpc, @Nullable ProtocolHandle<Rpc<A, R>> responseHandle) {
		...
		Task.runTxnAware(() -> o.sendOnlineRpc(rid, rpc, responseHandle, to, quiet));
	}
	...
	/** 直接通过 link 发送（事务感知）；忽略 quietWhenAbsent/timeout 选项。 */
