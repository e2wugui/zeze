...
public final class BLinkedMapKey implements Zeze.Transaction.BeanKey, Comparable<BLinkedMapKey> {
    ...
    public void buildString(StringBuilder _s_, int _l_) {
        _s_.append(Zeze.Util.Str.indent(_l_)).append("Zeze.Builtin.Collections.LinkedMap.BLinkedMapKey: {").append(System.lineSeparator());
        _l_ += 4;
    ...
    public boolean equals(Object _o_) {
        if (_o_ == this)
            return true;
        if (!(_o_ instanceof BLinkedMapKey))
            return false;
        //noinspection PatternVariableCanBeUsed
        var _b_ = (BLinkedMapKey)_o_;
