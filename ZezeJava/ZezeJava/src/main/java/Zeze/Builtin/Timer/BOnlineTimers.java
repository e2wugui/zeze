...
public final class BOnlineTimers extends Zeze.Transaction.Bean {
    ...
    private final Zeze.Transaction.Collections.PMap2<String, Zeze.Builtin.Timer.BOnlineCustom> _NamedNames;

    public Zeze.Transaction.Collections.PMap2<Long, Zeze.Builtin.Timer.BOnlineCustom> getTimerIds() {
        return _TimerIds;
    }

    public Zeze.Transaction.Collections.PMap2<String, Zeze.Builtin.Timer.BOnlineCustom> getNamedNames() {
        return _NamedNames;
    }
    ...
    public BOnlineTimers() {
        ...
        _NamedNames = new Zeze.Transaction.Collections.PMap2<>(String.class, Zeze.Builtin.Timer.BOnlineCustom.class);
    ...
    public void assign(BOnlineTimers other) {
        ...
        getNamedNames().clear();
        for (var e : other.getNamedNames().entrySet())
            getNamedNames().put(e.getKey(), e.getValue().copy());
    }
    ...
    public void buildString(StringBuilder sb, int level) {
        ...
        level += 4;
        for (var _kv_ : getNamedNames().entrySet()) {
            sb.append(Zeze.Util.Str.indent(level)).append('(').append(System.lineSeparator());
            sb.append(Zeze.Util.Str.indent(level)).append("Key").append('=').append(_kv_.getKey()).append(',').append(System.lineSeparator());
            sb.append(Zeze.Util.Str.indent(level)).append("Value").append('=').append(System.lineSeparator());
            _kv_.getValue().buildString(sb, level + 4);
