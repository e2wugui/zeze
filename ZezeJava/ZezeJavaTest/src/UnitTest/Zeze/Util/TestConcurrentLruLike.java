...
public class TestConcurrentLruLike {
	...
	public void testRemoveIfValueMatches() throws Exception {
		...
		var v1 = new Object();
		Assert.assertSame(v1, lru.getOrAdd("k", () -> v1));

		// 值不匹配：不删
		Assert.assertFalse(lru.remove("k", new Object()));
		Assert.assertSame(v1, lru.get("k"));

		// key 不存在：不删
		Assert.assertFalse(lru.remove("absent", v1));

		// 值匹配：删除成功，dataMap 与 lru 记录同步清理
		Assert.assertTrue(lru.remove("k", v1));
		Assert.assertNull(lru.get("k"));
		Assert.assertFalse(lruHotContains(lru, "k"));

		// 已删除后再条件删除返回 false
		Assert.assertFalse(lru.remove("k", v1));

		// 竞态场景：删除后同 key 已重建新值，按旧值条件删除不得误删新映射
		var v2 = new Object();
		Assert.assertSame(v2, lru.getOrAdd("k", () -> v2));
		Assert.assertFalse(lru.remove("k", v1));
		Assert.assertSame(v2, lru.get("k"));
		Assert.assertTrue(lru.remove("k", v2));
		Assert.assertNull(lru.get("k"));
		Assert.assertFalse(lruHotContains(lru, "k"));

		// 普通 remove(key) 行为不变：删除并清 lru 记录
		var v3 = new Object();
		Assert.assertSame(v3, lru.getOrAdd("k2", () -> v3));
		Assert.assertSame(v3, lru.remove("k2"));
		Assert.assertNull(lru.get("k2"));
		Assert.assertFalse(lruHotContains(lru, "k2"));
	}
