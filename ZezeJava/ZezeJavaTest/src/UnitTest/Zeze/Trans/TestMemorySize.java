...
public class TestMemorySize {
	...
	public void testMemorySize() throws Exception {
		...
		Assertions.assertEquals(1, App.Instance.demo_Module1.tMemorySize().getCacheSize());
		App.Instance.Zeze.newProcedure(() -> {
	...
	public void testMemoryRollback() {
		...
		Assertions.assertEquals(1, App.Instance.Zeze.newProcedure(() -> {
	...
	public void testMemoryCache() {
		...
		for (int i = 0; i < 0x40; i++) {
			int j = i;
			int r = i & 1;
			Assertions.assertEquals(r, App.Instance.Zeze.newProcedure(() -> {
				if (r == 1) {
					Bean1 v = null;
					switch ((j >> 1) & 3) {
					case 0:
			...
			Assertions.assertEquals(oldSize, App.Instance.demo_Module1.tMemorySize().getCacheSize());
		}
	}
}
