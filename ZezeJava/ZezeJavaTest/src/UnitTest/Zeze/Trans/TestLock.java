...
//#if RELEASE
		...
		Assertions.assertNull(keys.get(key1));
		keys.add(key1);

		demo.Module1.Key exist1 = keys.get(key1);
		Assertions.assertNotNull(exist1);
		Assertions.assertEquals(exist1, key1);

		demo.Module1.Key exist2 = keys.get(key2);
		Assertions.assertNotNull(exist2);
		Assertions.assertEquals(exist2, key1);
		...
		Assertions.assertNull(wref.get());

		demo.Module1.Key key3 = new demo.Module1.Key((short)1, "");
		System.out.println("test: is null.");
		Assertions.assertNull(keys.get(key3));
//#endif
	}
	...
	public final void test1() {
		...
		Assertions.assertEquals(lock1, lock2);

		Lockey lock1ref = locks.get(lock1);
		Assertions.assertEquals(lock1ref, lock1); // first Get. self

		Lockey lock2ref = locks.get(lock2);
		Assertions.assertEquals(lock2ref, lock1); // second Get. the exist

		TableKey tk3 = new TableKey(1, 2);
		Lockey lock3 = new Lockey(tk3);
		Lockey lock3ref = locks.get(lock3);
		Assertions.assertEquals(lock3ref, lock3);
