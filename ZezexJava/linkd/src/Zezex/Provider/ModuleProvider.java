...
public final class ModuleProvider extends AbstractModule {
	...
	public boolean ChoiceProvider(String serviceNamePrefix, int moduleId, int hash, Zeze.Util.OutObject<Long> provider) {
		...
		var volatileProviders = App.Instance.getServiceManagerAgent().getSubscribeStates().get(serviceName);
	...
	public boolean ChoiceProviderAndBind(int moduleId, Zeze.Net.AsyncSocket link, Zeze.Util.OutObject<Long> provider) {
		...
		var linkSession = (LinkSession)link.getUserState();
		provider.Value = 0L;
		var volatileProviders = App.Instance.getServiceManagerAgent().getSubscribeStates().get(serviceName);
		if (null == volatileProviders)
			return false;
		...
		if (ChoiceLoad(volatileProviders, provider)) {
			...
			var providerSocket = App.Instance.ProviderService.GetSocket(provider.Value);
			var providerSession = (ProviderSession)providerSocket.getUserState();
	...
	public int ProcessBindRequest(Protocol _rpc) {
		...
		if (rpc.Argument.getLinkSids().size() == 0) {
			var providerSession = (ProviderSession)rpc.Sender.getUserState();
			for (var module : rpc.Argument.getModules()) {
				if (getFirstModuleWithConfigTypeDefault() == 0 && module.getValue().getConfigType() == BModule.ConfigTypeDefault) {
					setFirstModuleWithConfigTypeDefault(module.getValue().ConfigType);
				}
				...
				var subState = App.getServiceManagerAgent().SubscrSbeService(serviceName, Zeze.Services.ServiceManager.SubscribeInfo.SubscribeTypeReadyCommit, providerModuleState);
...
//ORIGINAL LINE: private void UnBindModules(Zeze.Net.AsyncSocket provider, IEnumerable<int> modules, bool isOnProviderClose = false)
	...
	public int ProcessBroadcast(Protocol _protocol) {
		...
		App.Instance.LinkdService.Foreach((socket) -> {
				...
				var linkSession = (LinkSession)socket.getUserState();
	...
	public int ProcessSetUserState(Protocol _p) {
		...
		var socket = App.Instance.LinkdService.GetSocket(protocol.Argument.getLinkSid());
		var linkSession = (LinkSession)socket.getUserState();
		if (linkSession != null) {
	...
	public int ProcessReportLoad(Protocol _protocol) {
		...
		var providerSession = (ProviderSession)protocol.Sender.getUserState();
	...
	public int ProcessAnnounceProviderInfo(Protocol _protocol) {
		...
		var session = (ProviderSession)protocol.Sender.getUserState();
