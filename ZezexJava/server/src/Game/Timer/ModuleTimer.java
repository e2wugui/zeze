...
public class ModuleTimer extends AbstractModule {
    ...
    private ConcurrentHashMap<Long, Zeze.Util.Task> TimersLocal = new ConcurrentHashMap<>();
    ...
    private void ScheduleLocal(int serverId, long timerId, long nodeId, long delay, long period, String name) {
        if (period > 0) {
            TimersLocal.put(timerId, Zeze.Util.Task.schedule((This) -> TriggerTimerLocal(serverId, timerId, nodeId, name), delay, period));
        } else {
            TimersLocal.put(timerId, Zeze.Util.Task.schedule((This) -> TriggerTimerLocal(serverId, timerId, nodeId, name), delay));
        }
    }
