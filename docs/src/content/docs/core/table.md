...
每张表在本地维护一个 RocksDB 缓存表（`localRocksCacheTable`），构成三层存储架构：

```
内存缓存（TableCache, LRU）
    ↓ 淘汰
本地 RocksDB（localRocksCacheTable）
    ↓ 未命中
...
内存缓存受 LRU 容量限制，记录被淘汰后下次访问需回源远程数据库。远程数据库走网络 I/O，延迟在毫秒级；本地 RocksDB 读写本地 SSD，延迟在微秒级，差距可达两个数量级。本地 RocksDB 作为中间层，使得被内存淘汰的数据仍可快速恢复。本地 RocksDB 的生命期与进程一致——启动时创建、关闭时删除，不跨重启保留。

#### 一致性保证

引入本地缓存的核心难题是：如何保证本地 RocksDB 中的数据与远程数据库一致？尤其在分布式场景下，多个实例可能同时修改同一条记录。Zeze 的方案是不做独立的缓存失效策略（如 TTL），而是复用已有的全局缓存状态（State）来保证一致性：

- **写入路径**：Checkpoint 刷写脏记录时，在同一个 `flush` 调用中同时写入远程数据库和本地 RocksDB。远程写什么，本地就写什么。
- **读取路径**：记录从远程数据库加载后，立即写入本地 RocksDB 作为镜像。如果远程没有该记录，本地 RocksDB 中对应的条目也被删除。
- **失效路径**：在分布式场景下，其他实例修改某条记录后，当前实例的记录状态被 GlobalCacheManager 降级为 Invalid。下次访问时，框架发现状态为 Invalid，跳过本地 RocksDB 直接从远程数据库重新加载，并用最新数据覆盖本地 RocksDB。只有状态为 Share 或 Modify 时才信任本地 RocksDB 的数据。

这套机制的关键在于：本地 RocksDB 的有效性不由自身维护，而是由全局缓存状态统一保证。State 为有效时数据可信，State 为 Invalid 时强制回源。框架无需在本地 RocksDB 层引入额外的过期或失效策略，一致性由已有的缓存一致性协议统一管理。
